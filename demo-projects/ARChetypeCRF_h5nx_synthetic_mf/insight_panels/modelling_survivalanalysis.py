...
def create_visuals(df_map, df_forms_dict, dictionary, quality_report, filepath, suffix, save_inputs):
    ...
    max_lengthofstay = 15
    df_map["outco_lengthofstay"] = df_map["outco_lengthofstay"].fillna(max_lengthofstay)
    ...
    df_model["demog_agegroup"] = df_model["demog_age"].apply(lambda x: ">64" if (x > 64) else ("40-64" if (x > 45) else "0-39"))
    ...
    multivariate_cox_results.rename(
        columns={
            "covariate": "Variable",
            "HR": "HR (multi)",
            "p-value": "p-value (multi)",
            "CI_lower": "LowerCI (multi)",
            "CI_upper": "UpperCI (multi)",
        },
        inplace=True,
    )
    ...
    univariate_cox_results.rename(
        columns={
            "covariate": "Variable",
            "HR": "HR (uni)",
            "p-value": "p-value (uni)",
            "CI_lower": "LowerCI (uni)",
            "CI_upper": "UpperCI (uni)",
        },
        inplace=True,
    )

    cox_results = pd.merge(multivariate_cox_results, univariate_cox_results, on="Variable", how="outer")

    # ----- Is it possible to move this into execute_glm_regression
    cox_results["p-value (multi)"] = cox_results["p-value (multi)"].astype(float)
    cox_results["p-value (uni)"] = cox_results["p-value (uni)"].astype(float)

    cox_results["Variable"] = cox_results["Variable"].apply(lambda x: x.split("_True")[0])

    cox_results["Variable"] = cox_results["Variable"].apply(lambda x: x.split("___")[0]).map(
        dict(zip(["var" + str(n) for n in range(len(predictors))], predictors))
    ) + cox_results["Variable"].apply(lambda x: "___" + x.split("___")[-1] if "___" in x else "")
    # -----

    pvalue_significance = {"*": 0.05, "**": 0.01}
    cox_results_table = ia.regression_summary_table(
        cox_results.copy(), dictionary, pvalue_significance=pvalue_significance, result_type="HR"
    )

    table_key = "<br>".join([f"({k}) p < {str(v)}" for k, v in pvalue_significance.items()])
    ...
    columns = predictors + ["demog_agegroup", "outco_lengthofstay", "outco_binary_outcome"]

    df_km, df_risktable, p_value = ia.execute_kaplan_meier(
        df_model[columns], duration_col="outco_lengthofstay", event_col="outco_binary_outcome", group_col="demog_agegroup"
    )
