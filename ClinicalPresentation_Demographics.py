...
suffix='demogra'
Panel_title='Clinical Presentation: Demographics'
...
vari_list=getRC.getVariableList(redcap_api_key,['dates','demog','daily','outco'])
...
def visuals_creation(df_map):
    ...
    pyramid_chart = idw.dual_stack_pyramid(df_age_gender, base_color_map=color_map, graph_id='age_gender_pyramid_chart'+suffix)

    descriptive = ia.descriptive_table(df_map,correct_names,variables_binary,['age']+variables_number)
...
def create_modal():
    ...
    pyramid_chart,fig_table_symp=visuals_creation(df_map)
                ...
                dbc.AccordionItem(
                    ...
                    children=[
                        dbc.Tabs([
                            ...
                            dbc.Tab(dbc.Row([dbc.Col(pyramid_chart,id='age_gender_pyramid_'+suffix)]), label='Population Pyramid'),
...
def register_callbacks(app, suffix):
    ...
    @app.callback(
        [Output('age_gender_pyramid_'+suffix, 'children'),
    ...
    def update_figures(click, genders, age_range, outcomes, countries):
        ...
        pyramid_chart,fig_table_symp=visuals_creation(filtered_df)
        return [pyramid_chart,fig_table_symp]
