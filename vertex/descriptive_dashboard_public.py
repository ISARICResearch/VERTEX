...
#     try:
#         with open(os.path.join(path, "config_file.json"), "r") as json_data:
...
#             interpolated_rgb = [
#                 int(rgbs[i][j] + (float(step) / steps_per_transition) * (rgbs[i + 1][j] - rgbs[i][j])) for j in range(3)
#             ]
...
#             marker_opacity=0.5,
#             marker_line_width=0,
#             colorbar={"bgcolor": "rgba(0,0,0,0)", "thickness": 20, "ticklen": 1, "x": 1, "xref": "paper", "xanchor": "left"},
#         )
#     )
...
#             html.Div(
#                 [html.H1(title, id="title"), html.P(subtitle)],
...
#                 [
#                     html.Img(
#                         src="assets/logos/" + isaric_logo, className="img-fluid",
#                         style={"height": "7vh", "margin": "2px 10px"}
#                     ),
#                     html.P("In partnership with: ", style={"display": "inline"}),
#                 ]
#                 + [html.Img(src="assets/logos/" + logo, className="img-fluid", style=logo_style)
#                     for logo in partners_logo_list]
...
#         metadata_files = [file for file in metadata_files if file.endswith(".json") & file.startswith(graph_ids)]
...
#     for ii in range(len(buttons)):
#         buttons[ii]["visuals"] = tuple([visuals[id] for id in buttons[ii]["graph_ids"]])
#     return buttons
...
#         insight_children = [
#             dbc.Tabs(
#                 [dbc.Tab(dbc.Row([dbc.Col(dcc.Graph(figure=figure),
#                   id=id)]), label=label) for figure, id, label, _ in visuals],
#                 active_tab="tab-0",
#             )
#         ]
...
#         dbc.ModalBody(
#             [dbc.Accordion([dbc.AccordionItem(title="Insights", children=insight_children)], active_item="item-0")],
...
# def register_callbacks(app, buttons):
#     @app.callback(
#         [
...
#             button_ind = [ii for ii in range(len(buttons)) if buttons[ii]["suffix"] == suffix][0]
#             visuals = buttons[button_ind]["visuals"]
#             button = {"item": buttons[button_ind]["item"], "label": buttons[button_ind]["label"], "suffix": suffix}
...
#         "map_layout_center_longidute": -75,
#         "map_layout_zoom": 1.7,
#     }
...
#         map_center={"lat": config_dict["map_layout_center_latitude"], "lon": config_dict["map_layout_center_longitude"]},
#         margin={"r": 0, "t": 0, "l": 0, "b": 0},
#     )
#     fig = create_map(df_countries, map_layout_dict)

#     app.layout = define_app_layout(fig, buttons, map_layout_dict, config_dict["project_name"])
