...
def extend_dictionary(dictionary, new_variable_dict, data, sep="___"):
    ...
    for ind in new_dictionary.index:
        ...
        if parent not in new_dictionary["field_name"].values:
            if parent not in dictionary["field_name"].values:
                ...
                new_ind = dictionary.loc[(dictionary["field_name"] == parent), "index"].max()
                parent = dictionary.loc[new_ind, "field_name"]
                while parent in dictionary["parent"].values:
                    new_ind = dictionary.loc[(dictionary["parent"] == parent), "index"].max()
                    parent = dictionary.loc[new_ind, "field_name"]
                new_ind = new_ind + 0.1
        else:
            new_ind = new_dictionary.loc[(new_dictionary["field_name"] == parent), "index"].max() + 0.1
            # new_ind = new_ind + 0.1
    ...
    ind_list = [
        ind for ind in categorical_ind.loc[categorical_ind].index if new_dictionary.loc[ind, "field_name"] in data.columns
    ]
    for ind in ind_list:
        ...
        options = [y for y in options if (variable + sep + str(y)) not in new_dictionary["field_name"].values]
        add_options = pd.DataFrame(columns=dictionary.columns, index=range(len(options)))
        ...
        add_options["index"] = np.linspace(
            new_dictionary.loc[ind, "index"] + 0.2, np.ceil(new_dictionary.loc[ind, "index"]) - 0.1, len(options)
        )
...
def get_variables_by_section_and_type(
    ...
    exclude_suffix=["_units", "addi", "otherl2", "item", "_oth", "_unlisted", "otherl3"],
    ...
    if "section" in dictionary.columns:  # TODO: this should always be True in future
        ...
        include_ind = dictionary["field_name"].apply(lambda x: x.startswith(tuple(x + "_" for x in include_sections)))
    ...
    include_ind &= dictionary["field_name"].apply(lambda x: x.endswith(tuple(x for x in exclude_suffix))) == 0
...
def convert_categorical_to_onehot(df, dictionary, categorical_columns, sep="___", missing_val="nan", drop_first=False):
    ...
    for categorical_column in categorical_columns:
        onehot_columns = [var for var in df.columns if var.split(sep)[0] == categorical_column]
...
def convert_onehot_to_categorical(df, dictionary, categorical_columns, sep="___", missing_val="nan"):
    ...
    for categorical_column in categorical_columns:
        onehot_columns = list(df.columns[df.columns.str.startswith(categorical_column + sep)])
        # Preserve missingness
        df.loc[:, categorical_column + sep + missing_val] = (df[onehot_columns].any(axis=1) == 0) | (
...
def from_timeA_to_timeB(data, dictionary, timeA_column, timeB_column, timediff_column, timediff_label, time_unit="days"):
    if time_unit == "days":
        data[timediff_column] = (data[timeB_column] - data[timeA_column]).dt.days
    elif time_unit == "months":
        data[timediff_column] = (data[timeB_column].dt.year - data[timeA_column].dt.year) * 12 + (
    ...
    time_dict = {
        ...
        "form_name": dictionary.loc[(dictionary["field_name"] == timeB_column), "field_name"].values[0],
        "field_type": "numeric",
        "field_label": timediff_label,
        "parent": dictionary.loc[(dictionary["field_name"] == timeB_column), "parent"].values[0],
    }
...
def get_descriptive_data(
    ...
    df = convert_categorical_to_onehot(df, dictionary, categorical_columns=columns, sep=sep)

    if (by_column is not None) & (by_column not in df.columns):
        df = convert_onehot_to_categorical(df, dictionary, categorical_columns=[by_column], sep=sep)
...
def descriptive_table(
    data, dictionary, by_column=None, include_totals=True, column_reorder=None, include_raw_variable_name=False, sep="___"
):
    """
    ...
    table.loc[numeric_columns, "All"] = df[numeric_columns].apply(median_iqr_str, mfw=mfw)
    ...
    if by_column is not None:
        ...
        for value in choices_values:
            ...
            table.loc[numeric_columns, value] = df.loc[ind, numeric_columns].apply(median_iqr_str, mfw=mfw)
            table.loc[binary_columns, value] = df.loc[ind, binary_columns].apply(n_percent_str, mfw=mfw)
...
def descriptive_comparison_table(
    ...
    table["Variable"] = format_descriptive_table_variables(table_dictionary, sep=sep, binary_symbol="†").tolist()
    ...
    if by_column is not None:
        ...
        if len(by_column_values) == 2:
            ...
            pvalues.loc[binary_columns] = df[binary_columns].apply(lambda x: get_fisher_exact_pvalue(x, y), axis=0)
            table["p-value"] = pvalues.apply(format_pvalue, significance=pvalue_significance)
...
test,    (+) Median (IQR) | N, p-value using Mann-Whitney test<br>"""
    table_key += ",    ".join([f"({k}) p < {str(v)}" for k, v in pvalue_significance.items() if (pvalues < v).any()])
    return table, table_key
...
def format_descriptive_table_variables(dictionary, max_len=100, add_key=True, sep="___", binary_symbol="*", numeric_symbol="+"):
    ...
    name += (
        dictionary["field_label"]
        .apply(lambda x: x.split(":")[-1] if x.startswith("If") else x)
    ...
    if add_key is True:
        field_type = (
            ...
            .map({"categorical": f" ({binary_symbol})", "binary": f" ({binary_symbol})", "numeric": f" ({numeric_symbol})"})
            .fillna("")
        )
...
def format_variables(dictionary, max_len=40, sep="___"):
    ...
    name = ("<b>" + parent_name + "</b>, " + name) * answer_ind + ("<b>" + name + "</b>") * (~answer_ind)
...
def get_proportions(df, dictionary, max_n_variables=10, ignore_branching_logic=False, branching_logic="", sep="___"):
    if ignore_branching_logic:
        columns = [col for col in df.columns]
    else:
        dictionary_subset = dictionary.loc[(dictionary["branching_logic"] == branching_logic)]
        columns = [col for col in df.columns if col in dictionary_subset["field_name"].values]
        if len(columns) == 0:
            branching_logic = dictionary.loc[dictionary["field_name"].isin(df.columns), "branching_logic"]
            branching_logic = branching_logic.values[0]
            dictionary_subset = dictionary.loc[(dictionary["branching_logic"] == branching_logic)]
            columns = [col for col in df.columns if col in dictionary_subset["field_name"].values]
    proportions = df[columns].apply(lambda x: (x.sum() / x.count(), x.sum())).T.reset_index()

    proportions.columns = ["variable", "proportion", "count"]
    proportions = proportions.sort_values(by=["count"], ascending=False).reset_index(drop=True)
    ...
    proportions = proportions.sort_values(by=["proportion"], ascending=False).reset_index(drop=True)
...
def get_upset_counts_intersections(
    ...
    binary_columns = dictionary.loc[(dictionary["field_type"] == "binary"), "field_name"].tolist()
    ...
    intersections["label"] = intersections["index"].apply(lambda x: tuple(format_dict[y] for y in x))
    ...
    intersections["index_first"] = intersections[variables].idxmax(axis=1).map(variable_order_dict)
    ...
    intersections = intersections.sort_values(
        by=["count", "index_first", "index_last", "index_n"], ascending=[False, True, False, False]
    )
...
def get_modelling_data(
    ...
    df = convert_categorical_to_onehot(df, dictionary, categorical_columns=columns, drop_first=drop_first, sep=sep)
...
def variance_influence_factor_backwards_elimination(data, dictionary, predictors_list, sep="___"):
    df = data.copy()
    ...
    df = 1.0 * pd.concat([df[numeric_columns], df[categorical_columns]], axis=1).astype(float)
    ...
    with warnings.catch_warnings():
        ...
        iterative_vif["vif_iter_1"] = [
            variance_inflation_factor(df[keep_columns].values, ii) for ii in range(len(keep_columns))
        ]
    n = 1
    while (iterative_vif["vif_iter_" + str(n)] > 10).any():
        remove_column = iterative_vif.loc[iterative_vif["vif_iter_" + str(n)].idxmax(), "variable"]
        remove_column = remove_column.split(sep)[0]
        keep_columns = [col for col in keep_columns if (col.split(sep)[0] != remove_column)]
        n += 1
...
def remove_single_binary_outcome_predictors(data, dictionary, predictors_list, outcome_str):
    """
    ...
    result = (
        ...
        .apply(lambda x: x.isin([True]).any() & x.isin([False]).any(), include_groups=False)
        .T
    )
...
def regression_summary_table(
    table, dictionary, highlight_predictors=None, pvalue_significance=None, result_type="OddsRatio", sep="___"
):
    variables = table["Variable"].tolist()
    new_variables = (
        variables
        + dictionary.loc[
            ((dictionary["field_name"].isin(variables)) & (dictionary["parent"].isin([""]) == 0)), "parent"
        ].tolist()
    )
    ...
    nonrepeated_parent = dictionary.loc[variables].groupby("parent").apply(len, include_groups=False).eq(1)
    nonrepeated_parent = [
        p for p in nonrepeated_parent.loc[nonrepeated_parent].index if (dictionary.loc[p, "field_type"] != "section")
    ]
    variables = [var for var in variables if var not in nonrepeated_parent]
    dictionary = dictionary.reset_index()

    variables = dictionary.loc[(dictionary["field_name"].isin(variables)), "field_name"].tolist()
    ...
    for reg_type in ["multi", "uni"]:
        ...
        table[f"p-value ({reg_type})"] = table[f"p-value ({reg_type})"].apply(format_pvalue, significance=pvalue_significance)
    ...
    formatted_labels_v1 = format_descriptive_table_variables(dictionary, add_key=False, sep=sep)
    ...
    mapping_dict = {
        **dict(zip(dictionary.loc[v1_ind, "field_name"], formatted_labels_v1.loc[v1_ind])),
        **dict(zip(dictionary.loc[v2_ind, "field_name"], formatted_labels_v2.loc[v2_ind])),
    }
...
def execute_glmm_regression(
    ...
    if model_type.lower() == "linear":
        ...
        elr_model_obj = smf.mixedlm(formula=elr_formula_str, data=elr_dataframe_df, groups=elr_dataframe_df[elr_groups_str])
...
def execute_glm_regression(
    ...
    if model_type.lower() == "logistic":
        ...
        summary_df = summary_df.rename(
            columns={"index": "Study", "Odds Ratio": "OddsRatio", "IC Low": "LowerCI", "IC High": "UpperCI", "P>|z|": "p-value"}
        )
    else:
        summary_df = summary_table[["Coef.", "[0.025", "0.975]", "P>|z|"]].reset_index()
        summary_df = summary_df.rename(
            columns={"index": "Study", "Coef.": "Coefficient", "[0.025": "LowerCI", "0.975]": "UpperCI", "P>|z|": "p-value"}
        )
...
def execute_kaplan_meier(df, duration_col, event_col, group_col, alpha=0.05, n_times=5):
    ...
    for group in unique_groups:
        ...
        kmf.fit(group_data[duration_col], event_observed=group_data[event_col], label=str(group), alpha=alpha)
        ...
        survival_curve = pd.concat([kmf.survival_function_ * 100, ci_lower, ci_upper], axis=1)
        survival_curve = survival_curve.drop_duplicates().reset_index().rename(columns={"index": "timeline"})
    ...
    risk_counts = {
        group: [((df[group_col] == group) & (df[duration_col] >= t)).sum() for t in times] for group in unique_groups
    }
...
def impute_miss_val(df, dictionary, outcome_column="outco_binary_outcome", missing_threshold=0.7, verbose=False):
    """
    ...
    columns_to_drop = missing_proportions[(missing_proportions > missing_threshold)].index.tolist()
...
def rmv_low_var(df, dictionary, mad_threshold=0.1, freq_threshold=0.05, outcome_column="outco_binary_outcome", verbose=False):
    """
    ...
    single_value_columns = [col for col in single_value_columns if col not in keep_columns]
    ...
    exclude_binary_columns = [col for col in binary_columns if (binary_counts[col] < freq_threshold)]
    ...
    mad_series = (
        df[numeric_columns].apply(lambda x: x / x.abs().max(), axis=0).apply(lambda x: np.median(np.abs(x - np.median(x))))
    )
    exclude_numeric_columns = [col for col in numeric_columns if (mad_series[col] < mad_threshold)]
...
def rmv_high_corr(df, dictionary, outcome_column="outco_binary_outcome", correlation_threshold=0.5, verbose=False):
    """
...
only. More than two classes found."""
    ...
    if verbose:
        outcome_classes = dict(zip(label_encoder.classes_, range(len(label_encoder.classes_))))
...
ID if applicable)."""
    ...
    if verbose:
        ...
        sorted_features = sorted(selected_features, key=lambda x: feature_importance[x], reverse=True)
...
def create_grouped_results(selected_features, feature_importance, sep="___"):
    ...
    for field in categorical_fields:
        ...
        for feature in selected_features:
            if feature.startswith(field + sep):
                ...
                field_groups[field].append({"Feature": category, "Coefficient": coef, "AbsCoef": abs(coef)})

    # Process regular features
    for feature in regular_features:
        coef = feature_importance[feature]
        # For regular features, store coefficient directly
        field_groups[feature] = [{"Feature": "", "Coefficient": coef, "AbsCoef": abs(coef)}]
    ...
    for field in sorted_fields:
        if field in categorical_fields:
            ...
            results.append({"Feature": field, "Coefficient": field_groups[field][0]["Coefficient"]})
