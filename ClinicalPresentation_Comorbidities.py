...
suffix='comorb'
Panel_title='Clinical Presentation: Comorbidities'
...
vari_list=getRC.getVariableList(redcap_api_key,['dates','demog','comor','daily','outco'])
...
def visuals_creation(df_map):
    ...
    return fig_table_symp,freq_chart_comor,upset_plot_comor
...
def create_modal():
    ...
    fig_table_symp,freq_chart_comor,upset_plot_comor=visuals_creation(df_map)
                ...
                dbc.AccordionItem(
                    ...
                    children=[
                        dbc.Tabs([
                            ...
                            dbc.Tab(dbc.Row([dbc.Col(freq_chart_comor,id='freq_'+suffix)]), label='Comorbidities on presentation: Frequency'),
                            dbc.Tab(dbc.Row([dbc.Col(upset_plot_comor,id='upset_'+suffix)]), label='Comorbidities on presentation:Intersections')
...
def register_callbacks(app, suffix):
    ...
    def update_figures(click, genders, age_range, outcomes, countries):
        ...
        fig_table_symp,freq_chart_comor,upset_plot_comor=visuals_creation(filtered_df)
        return [fig_table_symp,freq_chart_comor,upset_plot_comor]
