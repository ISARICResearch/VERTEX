...
def create_visuals(df_map, df_forms_dict, dictionary, quality_report, filepath, suffix, save_inputs):
    ...
    df_table = ia.get_descriptive_data(df_map, dictionary, by_column=split_column, include_sections=["inter", "treat"])
    ...
    df_upset = ia.get_descriptive_data(
        df_daily, dictionary, include_sections=[section], include_types=["binary", "categorical"], include_subjid=True
    )
    ...
    proportions = ia.get_proportions(df_upset, dictionary)
    counts_intersections = ia.get_upset_counts_intersections(df_upset, dictionary, proportions=proportions)
    ...
    freq_chart_inter = idw.fig_frequency_chart(
        ...
        filepath=filepath,
        save_inputs=save_inputs,
        graph_id="fig_frequency_chart_inter",
        graph_label=section_name + ": Frequency*",
        graph_about=about,
    )
...
synthetic data. Results may not be clinically relevant or accurate."""
    ...
    return (fig_table, freq_chart_treat, upset_plot_treat, freq_chart_inter, upset_plot_inter, disclaimer)
