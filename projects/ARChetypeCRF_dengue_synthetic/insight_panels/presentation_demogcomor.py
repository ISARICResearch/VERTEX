...
def create_visuals(df_map, df_forms_dict, dictionary, quality_report, filepath, suffix, save_inputs):
    ...
    df_pyramid = ia.get_pyramid_data(df_map, column_dict, left_side="Female", right_side="Male")
    ...
    # split_column_order = ['Discharged', 'Death', 'Censored']
    df_table = ia.get_descriptive_data(
        df_map.loc[df_map["demog_sex"].isin(["Female", "Male"])],
        dictionary,
    ...
    table, table_key = ia.descriptive_comparison_table(
        df_table, dictionary, by_column=split_column, column_reorder=split_column_order
    )
...
synthetic data. Results may not be clinically relevant or accurate."""
    ...
    return (pyramid_chart, fig_table, fig_table_c, freq_chart_comor, upset_plot_comor, disclaimer)
