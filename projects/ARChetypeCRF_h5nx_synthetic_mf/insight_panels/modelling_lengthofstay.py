...
def create_visuals(df_map, df_forms_dict, dictionary, quality_report, filepath, suffix, save_inputs):
    ...
    linr_results = pd.merge(multivariate_linr_results, univariate_linr_results, on="Study", how="outer")

    # ----- Is it possible to move this into execute_glm_regression
    linr_results["p-value (multi)"] = linr_results["p-value (multi)"].astype(float)
    linr_results["p-value (uni)"] = linr_results["p-value (uni)"].astype(float)
    linr_results = linr_results.rename(columns={"Study": "Variable"})

    linr_results["Variable"] = linr_results["Variable"].apply(lambda x: "___".join(x.strip("]").split("[")).split("___True")[0])

    linr_results["Variable"] = linr_results["Variable"].apply(lambda x: x.split("___")[0]).map(
        dict(zip(["var" + str(n) for n in range(len(predictors))], predictors))
    ) + linr_results["Variable"].apply(lambda x: "___" + x.split("___")[-1] if "___" in x else "")
    # -----

    pvalue_significance = {"*": 0.05, "**": 0.01}
    linr_results_table = ia.regression_summary_table(
        linr_results.copy(), dictionary, pvalue_significance=pvalue_significance, result_type="Coefficient"
    )
