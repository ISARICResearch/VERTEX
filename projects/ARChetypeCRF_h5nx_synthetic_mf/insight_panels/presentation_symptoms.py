...
def create_visuals(df_map, df_forms_dict, dictionary, quality_report, filepath, suffix, save_inputs):
    ...
    sections = ["adsym", "sympt", "vital"]
    df_table = ia.get_descriptive_data(
        df_map, dictionary, by_column=split_column, include_sections=sections, exclude_negatives=False
    )
    ...
    section_name = "Signs and Symptoms"
    dictionary.loc[(dictionary["field_name"].str.startswith("adsym")), "field_label"] += " (onset to admission)"
    dictionary.loc[(dictionary["field_name"].str.startswith("sympt")), "field_label"] += " (first 24hr after admission)"
    df_upset = ia.get_descriptive_data(
        df_map, dictionary, include_sections=["adsym", "sympt"], include_types=["binary", "categorical"]
    )
    ...
    about = f"Intersection sizes of the most common \
    {section_name.lower()}"
...
synthetic data. Results may not be clinically relevant or accurate."""
    ...
    return (fig_table, freq_chart, upset_plot, disclaimer)
