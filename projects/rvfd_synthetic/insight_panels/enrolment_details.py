...
def create_visuals(df_map, df_forms_dict, dictionary, quality_report, filepath, suffix, save_inputs):
    ...
    df_sunburst = df_sunburst.sort_values(by="filters_country").reset_index(drop=True)

    enrolment_columns = ["subjid", "filters_country", "pres_enrolmentdate"]
    df_enrolment = df_map.loc[df_map["pres_enrolmentdate"].notna(), enrolment_columns].copy()
    groupby_columns = ["filters_country", "pres_enrolmentdate"]
    column = "pres_enrolmentdate"
    df_enrolment[column] = pd.to_datetime(df_enrolment[column], errors="coerce")
    df_enrolment[column] = df_enrolment[column].apply(lambda x: x.strftime("%m-%Y"))
    df_enrolment = df_enrolment[["subjid"] + groupby_columns].groupby(groupby_columns).nunique().reset_index()
    df_enrolment.rename(
        columns={"filters_country": "stack_group", "pres_enrolmentdate": "timepoint", "subjid": "value"}, inplace=True
    )

    # Convert 'timepoint' to datetime format and sort
    df_enrolment = df_enrolment.pivot_table(index="timepoint", columns="stack_group", values="value", aggfunc="sum").fillna(0)

    df_enrolment = df_enrolment.reset_index()
    df_enrolment["timepoint"] = pd.to_datetime(df_enrolment["timepoint"], errors="coerce", format="%m-%Y")
    df_enrolment = df_enrolment.sort_values("timepoint").reset_index(drop=True)
    df_enrolment["timepoint"] = df_enrolment["timepoint"].apply(lambda x: x.strftime("%m-%Y"))
