...
* If you want to run VERTEX in a **Docker container**, as described :ref:`here <running-bridge-in-docker>`, then the only requirement is `Docker Desktop <https://www.docker.com/products/docker-desktop/>`_. The Docker build process will ensure, via the `Dockerfile <https://github.com/ISARICResearch/VERTEX/blob/main/Dockerfile>`_, that all the app dependencies (and their sub-dependencies) are pre-installed inside the image used to run the container. **Docker is the recommended way of running VERTEX locally** as it results in more stable, reproducible builds.
