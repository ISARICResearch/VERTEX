...
For structuring analysis code, we borrow a concept called Reproducible Analytical Pipeline [#f3]_. This is intended to make analysis reproducible, efficient and auditable. We extend this concept to **Reusable Analytical Pipelines for Infectious Diseases (RAPIDs)** [#f4]_, which supports the transportation of these pipelines to answer the similar research questions in similar contexts.

**Static projects** do not execute the analysis pipelines, but reproduce the figures and tables directly from aggregated (not patient-level) data files [#f5]_. These files can be provided directly or can be automatically created when the users runs an analysis project.

.. _getting-started.installation-and-usage:

Installation and Usage
-----------------------
To use VERTEX locally, clone the VERTEX GitHub repository [#f6]_ and make sure that you have Python installed. There are several ways to run the VERTEX code, and depending on how your preferences for coding, you may also find it useful to have the following installed: git [#f7]_, Docker [#f8]_ and VSCode [#f9]_.

We will demonstrate how to run the code at the command line using Docker and in VSCode without Docker, though we recommend using Docker if possible [#f10]_.
...
Open the VERTEX folder within VSCode. You need to create an environment [#f11]_ from the VERTEX requirements. The simplest way to do this is Ctrl + Shift + p (or Command + Shift + p on MacOS), then click "Python: Create environment...", ".venv" and choose a Python version that is >3.9. This will install a virtual environment
