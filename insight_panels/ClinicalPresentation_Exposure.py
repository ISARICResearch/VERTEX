...
clinical_measure = 'Exposure in past 14 days'
...
def create_visuals(df_map):
    ...
    inclu_columns = ia.get_variables_from_sections(df_map.columns, ['expo14'])
    ...
    table = ia.reorder_descriptive_table(
        table, dictionary=dd, section_reorder=None)
    ...
    fig_table = idw.fig_table(
        ...
        graph_about='Summary of exposure in past 14 days.')
    ...
    # inclu_columns = [col for col in df_map.columns if col.startswith('adsym')]
    proportions, set_data = ia.get_proportions(df_map[inclu_columns], 'expo14')
    freq_chart = idw.fig_frequency_chart(
        proportions, dictionary=dd,
        title='Frequency of exposure in past 14 days',
        graph_id='freq_' + suffix,
        graph_label='Exposure: Frequency',
        graph_about='Frequency of the ten most common exposure')
    upset_plot = idw.fig_upset(
        set_data, dictionary=dd,
        title='Frequency of combinations of the five most common exposur in past 14 days',
        graph_id='upset_' + suffix,
        graph_label='Exposure: Intersections',
        graph_about='Frequency of combinations of the five most common exposure')
